...
function addNewCustomer (teamIdx?:number) {
    ...
    if (chooseTeam == 1) {
        ...
        let genfd3:number[] = []
        for (let index2 = 0; index2 < needFoodCnt3; index2++) {
            genfd3.push(randint(0, 5))
        }
        let customer2 = new Customer(img`
            . . . . . f f 4 4 f f . . . . .
            . . . . f 5 4 5 5 4 5 f . . . .
            . . . f e 3 3 3 3 3 3 e f . . .
            . . f b 3 3 3 3 3 3 3 3 b f . .
        ...
        customer2.team = chooseTeam
        grid.place(customer2, tiles.getTileLocation(6, 11))
        rightCustomers.push(customer2)
    }
}
class Customer extends Sprite {
    ...
    leaveStore() {
        ...
        for(let s = 0; s < this.wantFoodList.length; s++) {
            let fdsp222 = this.wantFoodList[s]
            fdsp222.destroy()
        }
        this.setImage(img`
            . . . . . . f f f f 4 4 f . . .
    ...
    disappear() {
        ...
        if(this.billspbg)
            this.billspbg.destroy()
        for(let t = 0; t < this.wantFoodList.length; t++) {
            let fdsp23 = this.wantFoodList[t]
            fdsp23.destroy()
        }
...
for (let p = 0; p <= initFoodStoreCnt - 1; p++) {
    foodsprite = sprites.create(foodlist[randint(0, 5)], SpriteKind.Food)
...
function updateCustomersLocation(team?:number) {
    if(team == 1) {
        for(let u = 0; u < 3; u++) {
            tiles.setWallAt(tiles.getTileLocation(1, 8+u), false)
        }
        for(let v = 0; v < leftCustomers.length; v++) {
            let csm3  = leftCustomers[v]
            if(v > 0) {
                tiles.setWallAt(tiles.getTileLocation(1, 8+v), false)
            }
            if(csm3.status == 0)
                csm3.status = -1
        }
        ...
        for(let w = 0; w < 3; w++) {
            tiles.setWallAt(tiles.getTileLocation(6, 8+w), false)
        }
        for(let a = 0; a < rightCustomers.length; a++) {
            let csm4  = rightCustomers[a]
            if(a > 0) {
                tiles.setWallAt(tiles.getTileLocation(6, 8+a), false)
            }
            if(csm4.status == 0)
                csm4.status = -1
        }
...
game.onUpdate(function () {
    ...
    for (let b = 0; b <= leftCustomers.length - 1; b++) {
        csm5 = leftCustomers[b]
        csm5.runAnimation()
    }
    for (let c = 0; c <= rightCustomers.length - 1; c++) {
        csm6 = rightCustomers[c]
        csm6.runAnimation()
    }
    if (state == 2) {
...
game.onUpdateInterval(1000, function () {
    ...
    for (let i = 0; i <= leftCustomers.length - 1; i++) {
        csm = leftCustomers[i]
        if (i < 2 && csm.status == 0) {
            csm.waitForFood()
        }
    }
    for (let j = 0; j <= rightCustomers.length - 1; j++) {
        csm8 = rightCustomers[j]
        if (j < 2 && csm8.status == 0) {
            csm8.waitForFood()
        }
    }
})
game.onUpdateInterval(4000, function () {
    if (storefoodLocations.length == 0) {
        ...
        if (radcol == grid.spriteCol(mySprite) && radrow == grid.spriteRow(mySprite)) {
            ...
            foodsprite2 = sprites.create(foodlist[randint(0, 5)], SpriteKind.Food)
            grid.place(foodsprite2, tiles.getTileLocation(radcol, radrow))
            tiles.setWallAt(tiles.getTileLocation(radcol, radrow), true)
